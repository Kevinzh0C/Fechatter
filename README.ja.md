...
- 🏗️ **マイクロサービスアーキテクチャ** - 異なる機能に対応する独立したサービスによるモジュラー設計
...
クライアントアプリケーション
    ↓
APIゲートウェイ (Pingora :8080)
    ↓
コアサービス:
├── Fechatterサーバー (:6688) → PostgreSQL, Redis, Meilisearch, NATS, S3
├── 通知サーバー (:6687) → Redis, NATS
├── ボットサーバー (:6686) → Redis, OpenAI API
└── 分析サーバー (:6690) → ClickHouse, NATS
