...
class SearchStateManager {
  ...
  getCachedResult(chatId, query, filters = {}) {
    ...
    if (this.isCacheValid(cacheEntry)) {
      console.log('🔍 [SearchStateManager] Cache hit:', cacheKey);
  ...
  setCachedResult(chatId, query, filters = {}, data) {
    ...
    console.log('🔍 [SearchStateManager] Cached result:', cacheKey);
  }
  ...
  cancelSearch(requestId) {
    ...
    if (activeRequest) {
      console.log('🔍 [SearchStateManager] Cancelling search:', requestId);
   ...
   */
  cancelAllSearches() {
    console.log('🔍 [SearchStateManager] Cancelling all active searches');
  ...
  async search(searchParams, searchFunction) {
    ...
    // 输入验证
    if (!chatId || typeof query !== 'string') {
      throw new Error('Invalid search parameters: chatId and query are required');
    }
    ...
    if (this.state.activeRequests.size >= this.state.concurrencyConfig.maxConcurrentRequests) {
      console.warn('🔍 [SearchStateManager] Too many concurrent searches, cancelling oldest');
    ...
    const cancelToken = {
      token: null,
      cancel: (reason) => {
        console.log('Search cancelled:', reason);
      }
    };
    ...
    try {
      ...
      console.log('🔍 [SearchStateManager] Starting search:', {
        requestId,
        chatId,
        query: trimmedQuery,
      ...
      if (!this.state.activeRequests.has(requestId)) {
        console.log('🔍 [SearchStateManager] Search request was cancelled:', requestId);
        return null;
      }
      ...
      console.log('🔍 [SearchStateManager] Search completed:', {
        requestId,
      ...
      if (error.message && error.message.includes('cancel')) {
        console.log('🔍 [SearchStateManager] Search was cancelled:', requestId);
        return null;
      }
      ...
      console.error('🔍 [SearchStateManager] Search failed:', {
        requestId,
        error: error.message
      });
  ...
  clearCache() {
    ...
    console.log('🔍 [SearchStateManager] Cache cleared');
  }
  ...
  reset() {
    ...
    console.log('🔍 [SearchStateManager] State reset');
  }
  ...
  updateConfig(newConfig) {
    ...
    console.log('🔍 [SearchStateManager] Config updated:', newConfig);
  }
}
