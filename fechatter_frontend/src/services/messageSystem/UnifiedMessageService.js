...
export class UnifiedMessageService {
    ...
    try {
      ...
      const getCurrentUserId = () => {
        try {
          ...
          if (import.meta.env.DEV) {
            console.warn('WARNING: [MessageStatus] Failed to get current user ID:', error);
          }
        }
        return null;
      };
      ...
      if (existingMessage) {
        ...
        if (isUserMessage && isFromRefresh) {
          if (import.meta.env.DEV) {
            console.log(`SUBSCRIPTION: [MessageStatus] User's message ${message.id} found in server refresh - marking as delivered`);
          }
          return {
            status: 'delivered',
      ...
      if (import.meta.env.DEV) {
        console.error('ERROR: [MessageStatus] Error determining message status:', error);
      }
      // 出错时返回默认状态
      return {
        status: 'sent'
  ...
  async fetchMessages(chatId, options = {}) {
    ...
    try {
      ...
      try {
        ...
        if (messages.length > 0 && !isPreload) {
          ...
          if (import.meta.env.DEV) {
            if (trackingId) {
              ...
              console.error(`ERROR: [UnifiedMessageService] Failed to start tracking for chat ${chatId} - no trackingId returned`);
            }
          ...
          if (import.meta.env.DEV) {
            console.log(`WARNING: [UnifiedMessageService] Skipping tracking for chat ${chatId}:`, {
              messageCount: messages.length,
              isPreload: isPreload,
        ...
        if (import.meta.env.DEV) {
          console.warn(`WARNING: [UnifiedMessageService] API call failed for chat ${chatId}:`, apiError.message);
        }
      ...
      if (import.meta.env.DEV) {
        console.error(`ERROR: [UnifiedMessageService] Failed to fetch messages for chat ${chatId}:`, error);
      }
  ...
  async fetchMoreMessages(chatId, options = {}) {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.error(`ERROR: [API Error] Failed to fetch more messages for chat ${chatId}:`, apiError.message);
      }
      ...
      try {
        ...
        if (import.meta.env.DEV) {
          if (trackingId) {
            ...
            console.error(`ERROR: [UnifiedMessageService] Failed to start tracking for more messages in chat ${chatId}`);
          }
        }
      } catch (trackingError) {
        if (import.meta.env.DEV) {
          console.error(`ERROR: [UnifiedMessageService] Failed to setup tracking for more messages:`, trackingError);
        }
