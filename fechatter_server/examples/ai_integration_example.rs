...
use fechatter_server::services::ai::{AiMessage, AiRole, AiServiceAdapter, OpenaiAdapter};
...
async fn fechatter_ai_operations() -> Result<()> {
    ...
    if let Ok(_) = std::env::var("OPENAI_API_KEY") {
        ...
        // Suggest replies
        let context =
            "User: Hey everyone! Just wanted to share that I got a promotion at work today!";
        ...
        match ai_service
            .moderate_content("This is appropriate content")
            .await
        {
