use anyhow::Result;
use chrono::Utc;
use fechatter_server::services::ai::{openai::OpenAIClient, SimpleTopicExtractor, SimpleWorkflow};
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let messages = vec![
        (
            1,
            101,
            "Hey everyone! Let's discuss the new project roadmap".to_string(),
            Utc::now(),
        ),
        (
            2,
            102,
            "Great idea! I think we should focus on user experience first".to_string(),
            Utc::now(),
        ),
        (
            3,
            103,
            "Agreed! Also, we need to consider security aspects".to_string(),
            Utc::now(),
        ),
        (
            4,
            101,
            "Security is definitely important. What about performance?".to_string(),
            Utc::now(),
        ),
        (
            5,
            104,
            "Performance optimization should be in phase 2".to_string(),
            Utc::now(),
        ),
        (
            6,
            102,
            "Should we schedule a meeting to discuss this further?".to_string(),
            Utc::now(),
        ),
        (
            7,
            103,
            "Yes, let's meet tomorrow at 2 PM".to_string(),
            Utc::now(),
        ),
        (
            8,
            101,
            "Perfect! I'll send out calendar invites".to_string(),
            Utc::now(),
        ),
    ];
...
async fn demo_with_real_ai(
    ...
    match topic_extractor.extract_topics(&message_data, 3).await {
        Ok(topics) => {
            for topic in topics {
                println!(
                    "Topic: {} (Keywords: {:?}, Messages: {})",
                    topic.name,
                    topic.keywords,
                    topic.message_ids.len()
                );
            }
        }
    ...
    match workflow.analyze_chat_complete(123, messages).await {
        Ok(result) => {
            ...
            println!(
                "   😊 Sentiment: {} ({})",
                result.sentiment.overall_label, result.sentiment.overall_score
            );
