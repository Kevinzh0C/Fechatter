...
pub async fn router_diagnostic_middleware(req: Request<Body>, next: Next) -> Response {
    ...
    if path.contains("/api/chat/") && path.contains("/") {
        ...
        if let Some(chat_id) = extract_chat_id_from_path(path) {
            ...
            warn!(
                "[ROUTER_DIAG] WARNING: Failed to extract chat ID from: {}",
                path
            );
        }
    }
    ...
    match status {
        ...
        StatusCode::UNAUTHORIZED => {
            warn!(
                "[ROUTER_DIAG] 🔐 UNAUTHORIZED: {} {} -> {} ({:?})",
                method, path, status, duration
            );
        }
        StatusCode::FORBIDDEN => {
            warn!(
                "[ROUTER_DIAG] 🛡️ FORBIDDEN: {} {} -> {} ({:?})",
                method, path, status, duration
            );
        }
        StatusCode::INTERNAL_SERVER_ERROR => {
            error!(
                "[ROUTER_DIAG] 💥 SERVER ERROR: {} {} -> {} ({:?})",
                method, path, status, duration
            );
        }
        _ => {
            info!(
                "[ROUTER_DIAG] RESPONSE: {} {} -> {} ({:?})",
                method, path, status, duration
            );
        }
    }
...
mod tests {
    ...
    fn test_extract_chat_id_from_path() {
        ...
        assert_eq!(
            extract_chat_id_from_path("/api/chat/456/messages"),
            Some(456)
        );
