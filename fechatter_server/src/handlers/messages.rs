...
pub async fn send_message_handler(
  ...
  if let Some(event_publisher) = get_unified_event_publisher(&state) {
    ...
    let chat_name = get_chat_name(&state, ChatId::from(chat_id)).await
...
pub async fn edit_message_handler(
  ...
  let original_content = get_message_content(&state, MessageId::from(message_id)).await
  ...
  if let Some(enhanced_publisher) = state.enhanced_event_publisher() {
    ...
    if let Err(e) = enhanced_publisher
      ...
      .await
    {
      tracing::warn!("Failed to publish edited message to notify_server for SSE: {}", e);
    } else {
      tracing::info!("Successfully published edited message to notify_server for SSE broadcasting");
    }
  }
  ...
  if let Some(event_publisher) = get_unified_event_publisher(&state) {
    ...
    if let Err(e) = event_publisher
      .publish_unified_message_edited(
        &message,
        editor_name,
        chat_name,
        workspace_id,
      ...
      .await
    {
      ...
      tracing::info!("Successfully published unified message edited event for message {}", message_id);
    }
  }
...
pub async fn delete_message_handler(
  ...
  if let Some(enhanced_publisher) = state.enhanced_event_publisher() {
    if let Err(e) = enhanced_publisher
      ...
      .await
    {
      tracing::warn!("Failed to publish deleted message to notify_server for SSE: {}", e);
    } else {
      tracing::info!("Successfully published deleted message to notify_server for SSE broadcasting");
    }
  }
  ...
  if let Some(event_publisher) = get_unified_event_publisher(&state) {
    ...
    if let Err(e) = event_publisher
      .publish_unified_message_deleted(
        &message,
        deleted_by_name,
        chat_name,
        workspace_id,
      ...
      .await
    {
      ...
      tracing::info!("Successfully published unified message deleted event for message {}", message_id);
    }
  }
...
pub async fn mark_messages_read_handler(
  ...
  if let Some(enhanced_publisher) = state.enhanced_event_publisher() {
    if let Err(e) = enhanced_publisher
      ...
      .await
    {
      tracing::warn!("Failed to publish read receipts to notify_server for SSE: {}", e);
    } else {
      tracing::info!("Successfully published read receipts to notify_server for SSE broadcasting");
    }
  }
  ...
  if let Some(event_publisher) = get_unified_event_publisher(&state) {
    let workspace_id = user.workspace_id.into();
    let message_ids: Vec<MessageId> = request.message_ids.iter().map(|&id| MessageId::from(id)).collect();
      ...
      .await
    {
      ...
      tracing::info!("Successfully published unified read receipt event for {} messages in chat {}", request.message_ids.len(), chat_id);
    }
  }
...
fn get_unified_event_publisher(state: &AppState) -> Option<&crate::services::infrastructure::event::DynEventPublisher> {
...
pub async fn get_message_mentions_handler(
  ...
  let mentions: Vec<MentionResponse> = mentions_data
    .into_iter()
    .map(|(user_id, username, fullname, mention_type)| MentionResponse {
      user_id,
      username,
      fullname,
      mention_type,
...
pub async fn get_unread_mentions_handler(
  ...
  let mentions: Vec<UnreadMentionResponse> = mentions_data
    .into_iter()
    .map(|(chat_id, message_id, content, sender_name, created_at, mention_type)| UnreadMentionResponse {
      chat_id,
      message_id,
      content,
      sender_name,
...
pub async fn get_detailed_message_receipts_handler(
  ...
  let receipts: Vec<DetailedReceiptResponse> = receipts_data
    .into_iter()
    .map(|(user_id, username, fullname, status, timestamp)| DetailedReceiptResponse {
      user_id,
      username,
      fullname,
      status,
