...
use crate::{
    ...
    error::AppError,
    services::application::workers::profile::service::{
        UserProfileService, UserProfileServiceTrait,
    },
    AppState,
};
...
pub async fn update_user_profile(
    ...
    let profile_service = if let Some(cache_service) = state.cache_service() {
        let unified_cache = std::sync::Arc::new(
            crate::services::infrastructure::cache::UnifiedCacheService::new(cache_service.clone()),
        );
...
pub async fn change_password_handler(
    ...
    let auth_service =
        crate::services::application::workers::auth::create_auth_user_service(&state);
...
pub async fn list_workspace_users_handler(
    ...
    let users: Vec<UserProfileResponse> = users_data
        .into_iter()
        .map(
            |(id, email, fullname, bio, avatar_url, status, created_at, _updated_at)| {
