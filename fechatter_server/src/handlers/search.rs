...
use crate::{
    error::AppError,
    services::application::workers::search::{
        MessageSearchResults, SearchApplicationServiceTrait, SearchPage, SearchableMessage,
    },
    AppState,
};
...
async fn secure_fallback_database_search(
  ...
  "#;
    ...
    let chat_access_row = sqlx::query(access_check_sql)
        ...
        .await
        .map_err(|e| {
            AppError::Internal(format!("Secure chat access verification failed: {}", e))
        })?;
