...
use crate::services::infrastructure::event::{
    AnalyticsConfig, EventTransport, LegacyEventPublisher, NatsAnalyticsPublisher, TransportFactory,
};
...
impl ProductionAuthServiceWrapper {
    pub fn new(app_state: &AppState) -> Self {
        ...
        let ha_auth_service =
            crate::services::application::workers::auth::HighAvailabilityAuthService::new(
...
pub async fn create_production_state(config: AppConfig) -> Result<AppState, AppError> {
    ...
    let event_publisher = if config.features.messaging.enabled {
        ...
        match TransportFactory::create_from_config(&config).await {
            ...
            Err(e) => {
                error!(
                    "ERROR: Failed to create NATS transport: {}. Using fallback.",
                    e
                );
    ...
    let mut application_services_builder = ApplicationServiceProvider::builder(
        Arc::new(pool.clone()),
