...
impl AppConfig {
  pub fn load() -> Result<Self> {
    if env::var("SHUTTLE").is_ok() || env::var("DATABASE_URL").is_ok() {
      info!("Running in Shuttle environment, using default configuration");
      return Ok(Self::default_shuttle_config());
    }
  ...
  fn default_config() -> Self {
    let db_url = env::var("DATABASE_URL")
      .unwrap_or_else(|_| "postgres://postgres:postgres@localhost:5432/fechatter".to_string());
  ...
  fn default_shuttle_config() -> Self {
    let db_url = env::var("DATABASE_URL")
      .unwrap_or_else(|_| "postgres://postgres:postgres@localhost:5432/fechatter".to_string());
    
    Self {
      server: ServerConfig {
        port: 8000, // Shuttle uses port 8000 by default
        db_url,
        base_dir: PathBuf::from("."),
      },
      auth: AuthConfig {
