  ...
  R: RefreshTokenRepository + Send + Sync,
{
  ...
  pub async fn refresh_token(
    ...
    let refresh_token = self
      ...
      .await?
      .ok_or_else(|| CoreError::Unauthorized("Invalid or expired refresh token".to_string()))?;
    ...
    let new_expires_at =
      now + chrono::Duration::seconds(super::token_repository::REFRESH_TOKEN_EXPIRATION as i64);
