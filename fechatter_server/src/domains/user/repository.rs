...
impl UserRepositoryImpl {
  ...
  pub async fn get_workspace_users(
    &self,
    workspace_id: WorkspaceId,
  ) -> Result<Vec<(i64, String, String, Option<String>, Option<String>, Option<String>, chrono::DateTime<chrono::Utc>, chrono::DateTime<chrono::Utc>)>, CoreError> {
    let rows = sqlx::query(
      r#"
      SELECT 
        id,
    ...
    let users = rows
      ...
      .map(|row| (
        ...
        row.get("status"),
        row.get("created_at"),
        row.get::<Option<chrono::DateTime<chrono::Utc>>, _>("updated_at").unwrap_or_else(|| row.get("created_at")),
      ))
      .collect();
