...
mod macros {
    ...
    macro_rules! call_service {
        ...
        ($service:expr, $trait:path, $method:ident, $arg1:expr, $arg2:expr) => {
            <crate::services::auth_service::AuthService as $trait>::$method(&$service, $arg1, $arg2)
                .await
        };
    ...
    macro_rules! verify_token {
        ($state:expr, $token:expr) => {
            <crate::AppState as fechatter_core::middlewares::TokenVerifier>::verify_token(
                &$state, $token,
            )
        };
    }
    ...
    mod tests {
        ...
        use anyhow::Result;
        use fechatter_core::{models::jwt::UserClaims, RefreshTokenService, TokenService};
