...
use crate::{error::ErrorOutput, AppState};

use axum::{response::Html, response::Json, routing::get, Router};
use fechatter_core::models::message::SearchResult;
use fechatter_core::{
    AuthUser, Chat, ChatType, ChatUser, CreateChat, CreateMessage, CreateUser, ListMessages,
    Message, SearchMessages, SearchableMessage, SigninUser, User, Workspace,
};
...
impl OpenApiRouter for Router<AppState> {
    fn openapi(self) -> Self {
        self.route("/api-docs/openapi.json", get(openapi_json))
