...
impl ChatServiceTrait for ChatService {
    ...
    async fn update_chat(
        ...
        let detail_view = ChatDetailView::from_chat(updated_chat, {
            ...
            let repo = crate::domains::chat::chat_member_repository::ChatMemberRepository::new(
                self.pool.clone(),
            );
            repo.get_member_count(chat_id.0)
                .await
    ...
    async fn list_chat_members(
        ...
        let member_repo = crate::domains::chat::chat_member_repository::ChatMemberRepository::new(
            self.pool.clone(),
        );
    ...
    async fn add_members(
        ...
        if !self.is_chat_admin(user_id, ChatId::new(chat_id)).await? {
            return Err(
                CoreError::Unauthorized("Only chat admin can add members".to_string()).into(),
            );
        }
    ...
    async fn remove_members(
        ...
        if !self.is_chat_admin(user_id, ChatId::new(chat_id)).await? {
            return Err(
                CoreError::Unauthorized("Only chat admin can remove members".to_string()).into(),
            );
        }
    ...
    async fn transfer_chat_ownership(
        ...
        let member_repo = Arc::new(
            crate::domains::chat::chat_member_repository::ChatMemberRepository::new(
                self.pool.clone(),
            ),
        );
...
impl ChatApplicationService {
    ...
    pub async fn delete_chat(&self, chat_id: i64, user_id: i64) -> Result<bool, AppError> {
        ...
        if !self.is_chat_creator(user_id, chat_id).await? {
            return Err(
                CoreError::Unauthorized("Only chat creator can delete chat".to_string()).into(),
            );
        }
    ...
    pub async fn get_chat(
        ...
        if !self.is_user_chat_member(user_id, chat_id).await? {
            return Err(CoreError::Unauthorized(
                "Only chat members can view chat details".to_string(),
            )
            .into());
        }
