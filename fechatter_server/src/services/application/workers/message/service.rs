...
use crate::services::infrastructure::flows::notifications::{
    create_notification_flow_service_with_nats, create_notification_service,
...
impl DualStreamDispatcher {
    ...
    pub async fn publish_realtime_event(&self, event: RealtimeEvent) -> Result<(), AppError> {
        ...
        let payload = serde_json::to_vec(&event).map_err(|e| {
            AppError::Internal(format!("Failed to serialize realtime event: {}", e))
        })?;
...
impl DualStreamMessageService {
    ...
    pub fn new_with_dependencies(
        ...
        let event_publisher = Arc::new(
            crate::services::infrastructure::event::legacy::EventPublisher::with_transport(
                transport,
            ),
        );
        let flow_service =
    ...
    pub async fn edit_message(
        ...
        if let Some(search_service) = self.search_service.as_ref() {
            ...
            let searchable_message =
                crate::services::application::workers::search::SearchableMessage {
    ...
    pub async fn from_state(state: &crate::AppState) -> Result<Self, AppError> {
        ...
        let repository =
            Arc::new(crate::domains::messaging::repository::MessageRepository::new(pool));
