...
impl ThirdPartyManager {
    ...
    pub async fn openai(&self) -> Result<Arc<OpenAIClient>, AppError> {
        self.openai_client
            .get_or_try_init(|| async {
                let openai_config = self.config.openai_config.as_ref().ok_or_else(|| {
    ...
    pub async fn cohere(&self) -> Result<Arc<CohereClient>, AppError> {
        self.cohere_client
            .get_or_try_init(|| async {
                let api_key = self.config.cohere_api_key.as_ref().ok_or_else(|| {
                    AppError::InvalidInput("Cohere API key not configured".to_string())
                })?;
