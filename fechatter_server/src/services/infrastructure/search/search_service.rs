...
impl SearchService {
    ...
    pub async fn index_messages(&self, messages: &[SearchableMessage]) -> SearchServiceResult<()> {
        ...
        for (batch_num, chunk) in messages.chunks(self.batch_size.get()).enumerate() {
            ...
            self.backend.index_documents(chunk).await.map_err(|e| {
                AppError::SearchError(format!("Batch {}: {}", batch_info.number, e))
            })?;
    ...
    pub async fn delete_messages(&self, message_ids: &[i64]) -> SearchServiceResult<()> {
        ...
        for (batch_num, chunk) in message_ids.chunks(self.batch_size.get()).enumerate() {
            ...
            self.backend.delete_documents(&ids).await.map_err(|e| {
                AppError::SearchError(format!("Batch {}: {}", batch_info.number, e))
            })?;
    ...
    fn build_filters_from_request(
        ...
        let chat_id = request.chat_id.ok_or_else(|| {
            AppError::InvalidInput(
                "chat_id is required (workspace-wide search not yet supported)".into(),
            )
        })?;
...
impl MeilisearchBackend {
    pub fn new(config: &SearchConfig, task_timeout: TaskTimeout) -> SearchServiceResult<Self> {
        ...
        let client = MeilisearchClient::new(&config.meilisearch_url, api_key).map_err(|e| {
            AppError::SearchError(format!("Failed to create Meilisearch client: {}", e))
        })?;
    ...
    async fn wait_for_task(&self, task: TaskInfo, operation: &str) -> SearchServiceResult<()> {
        let completed = task
            ...
            .await
            .map_err(|e| {
                AppError::SearchError(format!("Timeout waiting for {}: {}", operation, e))
            })?;
    ...
    async fn setup_index(&self) -> SearchServiceResult<()> {
        ...
        index
            ...
            .await
            .map_err(|e| {
                AppError::SearchError(format!("Failed to set searchable attributes: {}", e))
            })?;
    ...
    async fn ensure_index_exists(&self) -> SearchServiceResult<()> {
            ...
            .await
        {
            ...
            Err(meilisearch_sdk::errors::Error::Meilisearch(meilisearch_error)) => {
                ...
                if meilisearch_error.error_code
        ...
        self.wait_for_task(searchable_task, "searchable attributes setup")
            .await?;
        ...
        let filterable_task = index
            ...
            .await
            .map_err(|e| {
                AppError::SearchError(format!("Failed to set filterable attributes: {}", e))
            })?;

        self.wait_for_task(filterable_task, "filterable attributes setup")
            .await?;
        ...
        let sortable_task = index
            ...
            .await
            .map_err(|e| {
                AppError::SearchError(format!("Failed to set sortable attributes: {}", e))
            })?;

        self.wait_for_task(sortable_task, "sortable attributes setup")
            .await?;
