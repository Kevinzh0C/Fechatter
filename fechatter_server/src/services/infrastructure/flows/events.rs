  ...
  async fn publish_domain_event(&self, event: DomainEvent) -> Result<(), AppError> {
    if let Some(js) = &self.jetstream {
      ...
      let payload = serde_json::to_vec(&event)
        .map_err(|e| AppError::Internal(format!("Failed to serialize domain event: {}", e)))?;

      // JetStream persistent publish - ensure reliability
      let ack = js
        .publish(subject.clone(), payload.into())
        .await
        .map_err(|e| AppError::Internal(format!("Failed to publish domain event: {}", e)))?;

      // Wait for ack
      ack
        .await
        .map_err(|e| AppError::Internal(format!("Failed to get domain event ack: {}", e)))?;
