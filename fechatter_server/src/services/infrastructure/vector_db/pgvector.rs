...
use fechatter_core::{
    ...
    models::vector_db::{
        MessageEmbedding, MessageVectorRepository, MetadataFilter, VectorDatabase,
        VectorSearchResult,
    },
...
impl VectorDatabase for PgVectorDatabase {
    ...
    async fn update_metadata(&self, record_id: &str, new_metadata: Value) -> Result<(), CoreError> {
        ...
        let result = sqlx::query(
            "UPDATE message_embeddings SET metadata = $1, updated_at = $2 WHERE id = $3",
        )
        .bind(&new_metadata)
        .bind(update_time)
        .bind(record_id)
    ...
    async fn delete_by_filter(
        &self,
        filter_conditions: MetadataFilter,
