...
impl PineconeClient {
    pub fn new(
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| {
                VectorDbError::Permanent(format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    fn map_status_error(&self, status: StatusCode, error_text: String) -> VectorDbError {
        match status {
            ...
            StatusCode::FORBIDDEN => {
                VectorDbError::Permanent(format!("Access denied: {}", error_text))
            }
            ...
            s if s.is_server_error() => {
                VectorDbError::Transient(format!("Server error: {}", error_text))
            }
