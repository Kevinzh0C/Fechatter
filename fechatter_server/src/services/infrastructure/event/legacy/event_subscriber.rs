...
// };
use crate::services::infrastructure::cache::{
    DistributedLockCacheInvalidator, UnifiedCacheService,
};
use crate::AppError;
...
impl CacheEventSubscriber {
    ...
    async fn flush_batch(
        ...
        if operations > 0 {
            ...
            if prev_total > 0 {
                metrics_guard.avg_latency_us = (metrics_guard.avg_latency_us * prev_total
                    + latency)
                    / metrics_guard.total_invalidations;
            } else {
    ...
    async fn execute_invalidation(
        ...
        loop {
            ...
            let result: Result<(), AppError> = match &invalidation_type {
                ...
                InvalidationType::UserUpdated { user_id } => {
                    self.invalidator
                        .handle_user_updated_with_lock(*user_id)
                        .await
                }
            };
