...
mod examples {
    ...
    use crate::{
        ...
        services::infrastructure::event::auto_degradation::{
            ...
            publisher_integration::{
                clear_manual_override, get_current_backend, publish_message_created,
                publish_message_events_batch, publisher_health_check, switch_publisher_backend,
                PublisherFactory,
            },
        },
    };
    ...
    async fn example_basic_adaptive_usage() -> Result<(), AppError> {
        ...
        println!(
            "Publisher health: success_rate={:.2}%, latency={:.2}ms",
    ...
    async fn example_production_configuration() -> Result<(), AppError> {
        ...
        let health = publisher_health_check().await?;
        println!(
            "Final health status: success_rate={:.2}%, backend={}",
            health.success_rate * 100.0,
    ...
    async fn example_development_configuration() -> Result<(), AppError> {
        ...
        let health = publisher_health_check().await?;
        println!(
            "Development health check: healthy={}, backend={}",
    ...
    async fn example_environment_configuration() -> Result<(), AppError> {
        ...
        let health = publisher_health_check().await?;
        println!(
            "Environment-configured publisher: backend={}, healthy={}",
    ...
    async fn example_batch_publishing_with_monitoring() -> Result<(), AppError> {
        ...
        let health = publisher_health_check().await?;
        println!(
            "Batch publishing health: success_rate={:.2}%, latency={:.2}ms",
            health.success_rate * 100.0,
            health.average_latency_ms
        );
...
pub mod production_examples {
    use crate::{
        ...
        services::infrastructure::event::auto_degradation::{
            adaptive_publisher::{
                AdaptivePublisherConfig, DegradationThresholds, PublisherBackend,
                RecoveryThresholds,
            },
            publisher_integration::{publisher_health_check, PublisherFactory},
        },
    };
    use std::time::Duration;
    ...
    pub async fn health_monitoring_loop() -> Result<(), AppError> {
        loop {
            ...
            if !health.is_healthy {
                eprintln!(
                    "ALERT: Publisher unhealthy! Backend: {}, Success Rate: {:.2}%",
                    health.current_backend,
                    health.success_rate * 100.0
                );
            }
...
pub mod integration_patterns {
    use crate::{
        ...
        services::infrastructure::event::auto_degradation::{
            ...
            publisher_integration::{
                clear_manual_override, publish_message_created, switch_publisher_backend,
                PublisherFactory,
            },
        },
    };
    ...
    pub async fn gradual_migration_pattern(
