...
use crate::{
    ...
    services::infrastructure::event::{
        high_performance::{
            BackpressureConfig, CircuitBreakerConfig, PublisherConfig, RetryConfig,
        },
    ...
    async fn start_health_monitoring(&self) -> Result<(), AppError> {
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(
                safety_config.health_check_interval_seconds,
            ));
...
impl MigrationConfigLoader {
    ...
    pub fn load_with_overrides(
        base_config: EventPublisherMigrationConfig,
    ) -> EventPublisherMigrationConfig {
