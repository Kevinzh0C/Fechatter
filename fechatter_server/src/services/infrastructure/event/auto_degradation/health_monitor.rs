...
impl HealthMonitor {
    ...
    pub async fn record_operation(&self, success: bool, latency_ms: f64) {
        ...
        if self.config.enable_detailed_logging {
            debug!(
                "Health monitor recorded operation: success={}, latency={}ms",
                success, latency_ms
            );
        }
    }
    ...
    pub async fn calculate_metrics(&self) -> HealthMetrics {
        ...
        let success_count = recent_samples
            .iter()
        ...
        let mut latencies: Vec<f64> = recent_samples
            .iter()
    ...
    fn recommend_action(&self, reason: &DegradationReason) -> RecommendedAction {
        match reason {
            ...
            DegradationReason::ConnectionFailure { .. } => {
                RecommendedAction::CheckNetworkConnectivity
            }
