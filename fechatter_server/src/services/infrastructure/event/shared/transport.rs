...
impl NatsTransport {
  ...
  async fn record_success(&self, latency: Duration) {
    ...
    metrics.average_latency_ms = (metrics.average_latency_ms * (total - 1.0) + latency_ms) / total;
  }
...
impl EventTransport for NatsTransport {
  ...
  async fn publish_with_headers(
    ...
    self
      .publish_with_retry(subject, payload, Some(headers))
      .await
  }
...
impl TransportFactory {
  ...
  pub async fn create_nats_transport(
    ...
    let client = async_nats::connect(url)
      .await
      .map_err(|e| EventTransportError::Connection(format!("Failed to connect to NATS: {}", e)))?;
