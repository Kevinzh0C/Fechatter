...
impl OpenAIClient {
    ...
    pub fn new(config: OpenAIConfig) -> Result<Self, AppError> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| {
                AppError::AnyError(anyhow::anyhow!("Failed to create HTTP client: {}", e))
            })?;
    ...
    pub async fn generate_summary(&self, text: &str) -> Result<String, AppError> {
        ...
        let completion: CompletionResponse = response.json().await.map_err(|e| {
            AppError::AnyError(anyhow::anyhow!("Failed to parse OpenAI response: {}", e))
        })?;
