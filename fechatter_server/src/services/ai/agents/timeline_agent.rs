        ...
        for topic in topic_clusters {
            ...
            if !topic.message_ids.is_empty() {
                let entry = TimelineEntry {
                    ...
                    summary: format!(
                        "Discussion about {} with {} messages",
                        topic.topic,
                        topic.message_ids.len()
                    ),
                    participants: vec![], // LLM can fill this
                    message_range: (
                        *topic.message_ids.first().unwrap_or(&0),
                        *topic.message_ids.last().unwrap_or(&0),
                    ),
                    tags: topic.keywords,
    ...
    fn detect_high_activity_periods(
        ...
        if !high_activity_hours.is_empty() {
            ...
            for (hour_str, _) in high_activity_hours {
                let hour =
                    DateTime::parse_from_str(&format!("{} +00:00", hour_str), "%Y-%m-%d %H:%M %z")
                        .unwrap()
                        .with_timezone(&Utc);
