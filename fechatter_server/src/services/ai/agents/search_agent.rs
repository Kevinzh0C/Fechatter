...
impl SemanticSearchAgent {
    ...
    pub async fn index_messages(
        ...
        for (message_id, sender_id, content, timestamp) in messages {
            ...
            let chunks =
                self.chunk_message(&content, self.config.chunk_size, self.config.chunk_overlap);
    ...
    pub async fn search(
        ...
        let query_embedding = if let Some(cohere) = &self.cohere_client {
            cohere
                ...
                .next()
                .ok_or_else(|| {
                    AppError::ServerError("Failed to generate query embedding".to_string())
                })?
        } else {
                ...
                if let Some(message_id) = result.payload["message_id"].as_i64() {
                    ...
                    let metadata = fechatter_core::models::vector_db::Metadata {
                        chat_id: fechatter_core::ChatId(
                            result.payload["chat_id"].as_i64().unwrap_or(0),
                        ),
                        message_id: fechatter_core::MessageId(message_id),
                        sender_id: fechatter_core::UserId(
                            result.payload["sender_id"].as_i64().unwrap_or(0),
                        ),
        ...
        if self.config.use_reranking && self.cohere_client.is_some() {
            ...
            for (message_id, chunks) in message_map.iter() {
                ...
                sorted_chunks
                    .sort_by_key(|(_, meta)| meta.additional["chunk_index"].as_u64().unwrap_or(0));
            ...
            for (message_id, chunks) in message_map {
                ...
                let avg_score = sorted_chunks.iter().map(|(score, _)| score).sum::<f32>()
                    / sorted_chunks.len() as f32;
