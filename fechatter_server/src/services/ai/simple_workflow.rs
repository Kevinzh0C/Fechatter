...
impl SimpleWorkflow {
    ...
    async fn generate_chat_summary(
        ...
        fechatter_core::contracts::infrastructure::AIService::generate_summary(
            &*self.openai_client, &prompt
        ).await.map_err(|e| AppError::AnyError(anyhow::anyhow!("Summary generation failed: {}", e)))
    }
    ...
    async fn analyze_overall_sentiment(
        ...
        let sentiment = fechatter_core::contracts::infrastructure::AIService::analyze_sentiment(
            &*self.openai_client, &messages_text
        ).await.map_err(|e| AppError::AnyError(anyhow::anyhow!("Sentiment analysis failed: {}", e)))?;
    ...
    async fn create_simple_timeline(
        ...
        for topic in topics {
            let related_messages: Vec<_> = messages.iter()
                .filter(|(_, _, content, _)| {
