...
use ai_sdk::{AiAdapter, AiService, Message as AiMessage, Role as AiRole, OpenaiAdapter};
...
impl AiServiceAdapter {
    ...
    pub fn from_openai_config(config: OpenAIConfig) -> Result<Self, AppError> {
        ...
        let openai_adapter = OpenaiAdapter::new(
    ...
    fn convert_chat_messages(messages: Vec<ChatMessage>) -> Vec<AiMessage> {
        messages.into_iter().map(Self::convert_chat_message).collect()
    }
}
