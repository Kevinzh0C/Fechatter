...
impl RAGMessageIndexer {
    ...
    pub async fn build_chat_index(
        ...
        let daily_summaries = summary_agent
            .generate_rolling_summaries(chat_id, messages.clone(), 24, 24)
            .await?;

        // 2. Extract topics (let LLM do the work)
        let _message_data: Vec<(i64, String)> = messages
            .iter()
            .map(|(id, _, content, _)| (*id, content.clone()))
            .collect();
        let topic_clusters = summary_agent
            .identify_topic_clusters(chat_id, daily_summaries.clone(), messages.clone())
            .await?;

        // 3. Build timeline (let LLM enhance) - clone messages to avoid borrow issues
        let timeline = timeline_agent
